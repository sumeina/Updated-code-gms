<?php
// Read the latitude and longitude values from the Leaflet map$latArray = array();
$lngArray = array();
$cityArray = array();
$shortestPathCoords = array();
$shortestDistance = 0;

if (isset($_POST['latLngCityArray'])) {
    $latLngCityArray = json_decode($_POST['latLngCityArray'], true) ?? array();
    if (!empty($latLngCityArray)) {
        foreach ($latLngCityArray as $latLngCity) {
            if (!empty($latLngCity) && is_array($latLngCity) && count($latLngCity) > 0) {
                $lat = isset($latLngCity[0]) && !empty($latLngCity[0]) ? $latLngCity[0] : null;
                $lng = isset($latLngCity[1]) && !empty($latLngCity[1]) ? $latLngCity[1] : null;
                $city = isset($latLngCity[2]) && !empty($latLngCity[2]) ? $latLngCity[2] : null;
                if (!is_null($lat)) {
